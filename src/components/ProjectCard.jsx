...
export default function projectCard({ data }) {
  ...
  return (
    <div className=" mb-5 w-[400px] h-auto">
      <img src={data.projectImg} className="rounded  w-[250px] h-[250px] object-contain p-2 align-middle justify-center items-center" alt={data.projectTitle} />
      ...
      <div className="xl:pb-5 lg:pb-3 md:pb-2 pb-1 flex flex-wrap flex-row gap-2">
        {data.tags.map((tag, index) => (
          <button key={index} className="xl:font-medium font-bold flex-1  dark:text-[#E1E1FF] text-[#3730A3] xl:text-base text-xs border-2 border-[#3730A3]  bg-gray-100 rounded px-4 py-1">
            {tag}
          </button>
        ))}
      </div>
      <div className="flex justify-between">
        <a href={data.gitLink} target="_blank" rel="noopener noreferrer">
          <button className="xl:font-medium font-bold flex-1  dark:text-[#E1E1FF] text-[#3730A3] xl:text-base text-xs border-2 border-[#3730A3]  bg-gray-300 rounded px-4 py-1"> Github</button>
        </a>
        <a href={data.webLink} target="_blank" rel="noopener noreferrer">
          <button className="xl:font-medium font-bold flex-1  dark:text-[#E1E1FF] text-[#3730A3] xl:text-base text-xs border-2 border-[#3730A3]  bg-gray-300 rounded px-4 py-1	">View Site</button>
        </a>
      </div>
    </div>
  );
